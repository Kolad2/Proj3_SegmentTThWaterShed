...
FileNames = os.listdir("temp/StatisticCorData/")


Theta = {
    "Name": [],
    "Lognorm": {"Boolean": [], "s": [], "mu": []},
    "Expon": {"Boolean": [], "lambda": []},
    "Pareto": {"Boolean": [], "s": [], "xmin": []}}
...
for i0, FileName in enumerate(FileNames):
    ...
    matdict = scipy.io.loadmat("temp/StatisticCorData/" + FileName + "/" + FileName + "_S.mat", squeeze_me=True)
    ...
    Theta["Expon"]["Boolean"].append(((f_0_low[1:] < f[1][1:-1]) & (f[1][1:-1] < f_0_height[0:-1])).all())
    Theta["Expon"]["lambda"].append(theta[1][1])
    ...
    Theta["Pareto"]["s"].append(theta[2][0])
    Theta["Pareto"]["xmin"].append(theta[2][2])
...
header = ["Номер Образца", "Lognorm boolean", "Lognorm s", "Lognorm mu", "Expon boolean", "Expon lambda", "Pareto boolean", "Pareto s", "Pareto xmin"]
...
with open('temp/StatisticResult.csv', 'w', encoding='UTF8', newline='') as f:
    ...
    for i in range(0,len(Theta["Lognorm"]["s"])):
        data = [Theta["Name"][i],
                ...
                Theta["Expon"]["Boolean"][i],
                round(Theta["Expon"]["lambda"][i], 2),
                Theta["Pareto"]["Boolean"][i],
                round(Theta["Pareto"]["s"][i], 2),
                round(Theta["Pareto"]["xmin"][i], 2)]
        # write the data
